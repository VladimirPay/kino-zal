...
// отметили «нравится», уже точно так же отметил ЛЮБОЙ другой пользователь
// сайта (не обязательно друг) — это «совпадение», и вы оба сразу об этом
// узнаёте (совпадение вычисляет и запоминает сама база данных — функция
// check_swipe_match() из миграции social-upgrade-3.sql, обновлена в
// social-upgrade-9.sql, чтобы матчи считались между всеми, а не только
// друзьями — поэтому сайт не должен читать чужие свайпы напрямую, только
// сам факт совпадения).
...
export async function loadMatchesList() {
  ...
  if (!data || !data.length) {
    el.innerHTML = '<p class="empty-note">Пока совпадений нет — свайпайте карточки ниже, совпадение может случиться с любым пользователем сайта.</p>';
    return;
  }
